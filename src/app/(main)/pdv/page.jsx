                    ...
                    <div className="bg-(--background) rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                            {cuenta?.items?.map((it, idx) => {
                                const productoInfo = productos.find(p => p.idProducto === it.idProducto);
                                const nombreProducto = it.producto?.nombre || productoInfo?.nombre || `Producto ${it.idProducto}`;
                                return (
                                    <div key={idx} className="flex justify-between items-start border-b border-gray-50 dark:border-gray-900 pb-3">
                                        <div className="space-y-1">
                                            <p className="font-semibold">{nombreProducto} x{it.cantidad}</p>
                                            {it.nota && <p className="text-xs text-gray-600 dark:text-gray-400 italic bg-gray-50 dark:bg-gray-900/20 px-2 py-0.5 rounded">Nota: {it.nota}</p>}
